...
are OUT for captions. Bold white text; the key/spoken word pops by COLOUR — `green` `#2BE86A`
(brand) or `yellow` `#FFD700` (industry). Plates/inversion stay for OTHER components
(lower-third, callout) — only captions changed.
